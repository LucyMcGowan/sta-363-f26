...
## Application exercise 4, part 1 {.small}

**1.** Go to [Google Colab](https://colab.research.google.com)

**2.** Split `Default` the same way as always:
...
::: {.question}
That number is the proportion of the test set that defaulted. If you
predicted "no default" for every single person, what would your error rate
be?

Write it down. It is the bar every model today has to clear.
:::
...
**1.** Compare an unscaled fit to a scaled one, both with $K = 25$:

```{python}
#| eval: false
unscaled = KNeighborsClassifier(n_neighbors=25).fit(X_train, y_train)
round(1 - unscaled.score(X_test, y_test), 4)
```
...
round(1 - scaled.score(X_test, y_test), 4)
```

::: {.question}
Which did better?

The unscaled error should look familiar. Compare it to the number you
wrote down above. What is the unscaled model actually predicting?
:::

## Application exercise 4, part 1
...
::: {.question}
Two customers differ by \$1,000 in `balance` and \$1,000 in `income`.
Before scaling, how much does each difference contribute to the distance
between them? Which predictor is deciding who counts as a neighbor?
:::
...
## Application exercise 4, part 2 {.small}

**1.** Sweep $K$ over `[1, 3, 5, 10, 25, 50, 100, 200, 500]`, recording
training and test error at each value.

::: {.question}
What is the training error at $K = 1$? Why is that value guaranteed
before you run anything?

2. Which $K$ minimizes test error?  
3. What happens to test error at very large $K$, and why?  
:::

## Application exercise 4, part 2

Plot it against $1/K$ so that flexibility increases left to right.

::: {.question}
Compare the shape of your curves to Figure 2.17 in the textbook. Where do
they agree, and where does real data refuse to look like the textbook?
:::
...
## Application exercise 4, part 3 {.small}

The bank cannot contact everyone. Suppose they will reach out to the 100
customers the model considers most at risk.

::: {.question}
1. What fraction of the top 100 actually defaulted?  

Compare that to the overall default rate in the test set. How many times
better than random is the model at finding defaulters?
:::

## Application exercise 4, part 3

::: {.question}
The model's error rate was only slightly better than "predict no for
everyone." Yet this number is dramatically better than chance. How can
both be true?
:::
